...
2、求值表达式，若为true，执行循环体，若为false则跳出循环
...
幂等请求：可以随意执行多次，且结果不会有影响

2、get是读取数据，一般会对其进行缓存处理，post是提交数据，一般不会对其进行缓存处理

3、get用url来传输数据，post用请求体来传输数据

4、get的请求参数会被保存到浏览器记录中，post不会保存
